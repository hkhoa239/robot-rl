"""
Generates six publication-quality visualizations from the PPO delayed reward
...
3. Value Prediction     – mean V(s) trajectory over an episode
4. Credit Assignment    – TD-error magnitude heatmap (log scale)
...
def plot_reward_timeline(R):
    ...
    for i, name in enumerate(CONFIGS):
        ...
        # Annotate bars that overflow the y range
    ...
    fig.suptitle("1 · Reward Timeline (PPO): Delivered vs Original  (y clipped ±30)",
...
def plot_return_variance(R):
    # Pre-compute smoothed variance curves for all configs
    curves = {}
    for name in CONFIGS:
    ...
    # Top: log-scale — all curves visible despite orders-of-magnitude gap
    for name in CONFIGS:
        x, y = curves[name]
    ...
    ax_top.set_title("2 · Return Variance Over Episode Timesteps (PPO)",
                     fontweight="bold")
    ax_top.legend(fontsize=8)
    ax_top.grid(True, alpha=0.3, which="both")

    # Bottom: linear zoom — exclude sparse to reveal fine structure
    for name in CONFIGS:
        if name == "sparse":
            continue
        x, y = curves[name]
...
def plot_value_prediction(R):
    ...
    for name in CONFIGS:
        ...
        for t in range(max_len):
            ...
            mean_v.append(np.mean(vals) if vals else 0.0)
        sm = _smooth(mean_v, w=5) if len(mean_v) > 5 else np.array(mean_v)
        ax.plot(np.arange(len(sm)) + 2, sm,
                color=COLORS[name], linewidth=1.8, label=LABELS[name])
    ax.set_xlabel("Timestep")
    ax.set_ylabel("V(s)")
    ax.set_title("3 · Value Prediction: Mean V(s) Over Episode (PPO)",
                 fontweight="bold")
...
def plot_credit_assignment(R):
    ...
    fig.suptitle("4 · Credit Assignment: TD-Error Magnitude – log scale (PPO)",
                 fontsize=13, fontweight="bold")
...
def plot_robustness(R):
    ...
    ax.set_title("5 · Delayed Reward Robustness: Performance vs Delay (PPO)",
                 fontweight="bold")
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_learning_curve(R):
    ...
    ax.set_title("6 · Learning Curve: PPO Under Different Reward Structures",
                 fontweight="bold")
...
def print_summary(R):
    print("\n" + "=" * 70)
    print("  Delayed Reward Experiment (PPO) – Summary")
    print("=" * 70)
...
# ─── Main ────────────────────────────────────────────────────────────────────
def main():
    print("[INFO] Loading PPO experiment results …")
    R = load_results()
