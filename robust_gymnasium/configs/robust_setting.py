...
def parse_robust_args():
    """Parse robust configuration args, ignoring unrecognized arguments.

    This is needed for Jupyter notebook compatibility where sys.argv contains
    the kernel connection file path, which argparse would otherwise try to
    parse as a float argument value and fail.
    """
    parser = get_config()
