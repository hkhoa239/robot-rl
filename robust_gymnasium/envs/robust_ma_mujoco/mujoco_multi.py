...
from robust_gymnasium.envs.robust_ma_mujoco.coupled_half_cheetah import (
    CoupledHalfCheetahEnv,
)
from robust_gymnasium.envs.robust_ma_mujoco.obsk import (
    Node,
    build_obs,
    get_joints_at_kdist,
    get_parts_and_edges,
...
class MultiAgentMujocoEnv(pettingzoo.utils.env.ParallelEnv, robust_gymnasium.Env[NDArray[np.float64], NDArray[np.float32]]):
    ...
    def __init__(
        ...
        gym_env: robust_gymnasium.envs.robust_mujoco.mujoco_env.MujocoEnv | None = None,
        **kwargs,
    ):
        """Init.
    ...
    def _create_base_gym_env(
        ...
        if scenario in _MUJOCO_GYM_ENVIROMENTS:
            ...
            single_agent_env = robust_gymnasium.make(
                "Ant-v5s", xml_file=asset_path, **kwargs, render_mode=render_mode
            )
            ...
            single_agent_env = robust_gymnasium.make(
                "Swimmer-v5s", xml_file=asset_path, **kwargs, render_mode=render_mode
            )
            os.remove(asset_path)
            return single_agent_env
    ...
    def map_local_actions_to_global_action(
        ...
        for agent_id, partition in enumerate(self.agent_action_partitions):
            for act_index, body_part in enumerate(partition):
                # print("body_part.act_ids-------:", body_part.act_ids)
                # print("multi-mujoco-actions------:", actions)
                # print("agent_id-------:", agent_id)
                # print("self.possible_agents[agent_id]--------:", self.possible_agents[agent_id])
                # print("actions[self.possible_agents[agent_id]]------:", actions[self.possible_agents[agent_id]])
                # print("self.possible_agents[agent_id]------:", actions[self.possible_agents[agent_id]])
                assert np.isnan(
                ...
                global_action[body_part.act_ids] = actions[
                    self.possible_agents[agent_id]][act_index]
    ...
    def render(self):
        ...
        Returns:
            The same return value as the single agent robust_gymnasium.MuJoCo
