...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    ...
    ```
    env = robust_gymnasium.make('HalfCheetah-v2')
    ```

    v3 and v4 take robust_gymnasium.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = robust_gymnasium.make('HalfCheetah-v4', ctrl_cost_weight=0.1, ....)
    ```
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, robust_input):
        ...
        info = {
            ...
            "reward_l1": -np.abs(x_velocity - self._goal_vel),
            "reward_run": forward_reward,
            "reward_ctrl": -ctrl_cost,
            "reward": -np.abs(x_velocity - self._goal_vel), # -ctrl_cost ,
            "cost": -np.abs(observation[0] - height),
            "cost_l1": -np.abs(observation[0] - height)
        }
