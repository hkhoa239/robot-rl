...
class HumanoidEnv(MujocoEnv, utils.EzPickle):
    ...
    *`contact_cost_weight` * sum(external contact force<sup>2</sup>)* to the interval specified by `contact_cost_range`.
    ...
    ```
    env = robust_gymnasium.make('Humanoid-v4')
    ```

    v3 and v4 take robust_gymnasium.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = robust_gymnasium.make('Humanoid-v4', ctrl_cost_weight=0.1, ....)
    ```
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, robust_input):
        ...
        info = {
            ...
            # "cost": ctrl_cost,
            "reward1": forward_reward, # todo: reward objective one
            "reward2": healthy_reward, # todo: reward objective two
            "reward": forward_reward,  # todo: reward
            "cost": -costs - cost_healthy, # todo: safety
        }
