...
import random
from robust_gymnasium.configs.robust_setting import parse_robust_args
args = parse_robust_args()
...
class Walker2dEnv(MujocoEnv, utils.EzPickle):
    ...
    by adding another set of legs making it possible for the robot to walker_seed1 forward instead of
    hop. Like other Mujoco environments, this environment aims to increase the number of independent state
    and control variables as compared to the classic control environments. The walker_seed1 is a
    two-dimensional two-legged figures that consist of four main body parts - a single torso at the top
    ...
    Observations consist of positional values of different body parts of the walker_seed1,
    ...
    represent the x-coordinates of the top of the walker_seed1.
    ...
    - *healthy_reward*: Every timestep that the walker_seed1 is alive, it receives a fixed reward of value `healthy_reward`,
    ...
    *dt = 4 * 0.002 = 0.008*. This reward would be positive if the walker_seed1 walks forward (right) desired.
    - *ctrl_cost*: A cost for penalising the walker_seed1 if it
    ...
    *`ctrl_cost_weight` * sum(action<sup>2</sup>)* where *`ctrl_cost_weight`* is
    ...
    ### Episode End
    The walker_seed1 is said to be unhealthy if any of the following happens:

    1. Any of the state space values is no longer finite
    2. The height of the walker_seed1 is ***not*** in the closed interval specified by `healthy_z_range`
    ...
    2. Termination: The walker_seed1 is unhealthy
    ...
    ```
    env = robust_gymnasium.make('Walker2d-v4')
    ```

    v3 and beyond take robust_gymnasium.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = robust_gymnasium.make('Walker2d-v4', ctrl_cost_weight=0.1, ....)
    ```
    ...
    | `terminate_when_unhealthy`                   | **bool**  | `True`           | If true, issue a done signal if the z-coordinate of the walker_seed1 is no longer healthy                                                                               |
    | `healthy_z_range`                            | **tuple** | `(0.8, 2)`       | The z-coordinate of the top of the walker_seed1 must be in this range to be considered healthy                                                                          |
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, robust_input):
        ...
        if args.noise_factor == "reward":
            if args.noise_type == "gauss":
                info["reward"] = info["reward"] + random.gauss(mu, sigma)  # robust setting
            elif args.noise_type == "shift":
                info["reward"] = info["reward"] + args.noise_shift
            else:
